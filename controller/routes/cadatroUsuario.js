...
app.get('/login', (req, res) => {
  res.render('Login', { title: 'Cadastro', mensagemLogin: null });
});

app.post('/login/executar', function(req, res){
  console.log(req.body.nome);
  console.log("oo------");
  console.log(usuarioBanco.selectUsuario({nome:req.body.nome, senha:seguranca.ocultarSenha( req.body.senha)}));
  console.log("aaa------");
    if(usuarioBanco.selectUsuario({nome:req.body.nome, senha:seguranca.ocultarSenha( req.body.senha)})){
      res.send("Eba!!!");
    }else{
    res.send("aeee!!!");
    }
